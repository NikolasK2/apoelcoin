...
#if defined(NDEBUG)
# error "Apoelcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x805688e44dab785ccb5b9ee01e6f53722a4b20eee44a6a55d045644a901f82d6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Apoelcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Apoelcoin Signed Message:\n";
...
{
    // Apoelcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Apoelcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Apoelcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Apoelcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Apoelcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Apoelcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4fd826dcd5084b1b89abe0454edb90a342e0f6dec7545402c5d0b3307cba7236");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b3064ea95a3bc5b3b89c359bac20bc9cfc4a6bf60ef20c518ec056cd77b3e253565005d3dd334020d5bcc079e5d7ff1fc6bda2c0d76258b455bd9f488e903040") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614437914;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086744338;

        if (fTestNet)
        {
            block.nTime    = 1614437882;
            block.nNonce   = 386880415;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x52d08622ebb564921f49659c1c705c9a50d0d44ba044e234a0b9cc82bee0c239"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xca, 0xb9, 0xd6 }; // Apoelcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ApoelcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ApoelcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ApoelcoinMiner(CWallet *pwallet)
{
    printf("ApoelcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("apoelcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ApoelcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ApoelcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ApoelcoinMiner, pwallet));
}
